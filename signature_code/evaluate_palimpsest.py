...
if __name__ == '__main__':
    ...
    for setting in ('all', 'cancertype', 'prefit'):
        ...
        sml = list()
        sml.append(ccf_file.Clonality.nunique())
        sml.append(np.nan)
        sml.append(np.nan)
        sml.append(score1B(sim_data_obj, ccf_file.Clonality.nunique()))
        ...
        for v in (auc, accuracy, sensitivity, specificity, precision):
            sml.append(v)
        signature_mut = np.array([sig_names.index(s) for s in ccf_file.origin])
