...
for (suffix in c('prefit', 'all', 'cancertype')) {
    start_time <- Sys.time()
    if (suffix == 'prefit') {
        signatures_exp <- deconvolution_fit(vcf = vcf_ccf, type = "SNV",
                                           ...
                                           resdir = resdir)
        if (sum(signatures_exp$sig_props>0)==1) {
          # one signature only doesn't work
          new_sig = sample(names(signatures_exp$sig_props)[signatures_exp$sig_props==0], 1)
          signatures_exp$sig_props[new_sig] = 1
