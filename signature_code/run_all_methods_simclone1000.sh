...
i=1
for PARAM in samplename randomstate
# samplename="Sim_500_19"
do i=$((i+1)) ; eval $PARAM=$(grep "^$PBS_ARRAYID," $INPUT_FILE | cut -d',' -f$i) ; done

sed 's/^\#\#/\&/g' data_simu_pcawg/testing_SimClone1000_truth/${samplename}/simulated_0001/simulated_0001_0001/snv/icgc_consensus/simulated_0001_0001.vcf >  data_simu_pcawg/testing_SimClone1000_truth/${samplename}/simulated_0001/simulated_0001_0001/snv/icgc_consensus/simulated_0001_0001_clean.vcf

source /data/users/jabecass/dl_tools_centos/miniconda3_activate

source activate my_python36_centos
./signature_code/get_input_simclone1000.py $samplename $randomstate
folder_path_cst=SimClone1000/$samplename\_cst
folder_path_var=SimClone1000/$samplename\_var

./signature_code/run_clonesig_phylo500.py $folder_path_cst
./signature_code/run_clonesig_phylo500.py $folder_path_var
source deactivate

nb_mut=`wc -l $folder_path_cst/input_t.tsv | cut -d " " -f 1`

if [ $nb_mut -lt 10001 ]; then 
# run sciclone
...
source activate my_python36_centos
./signature_code/evaluate_simclone1000.py $folder_path_cst
./signature_code/evaluate_simclone1000.py $folder_path_var
source deactivate
fi
