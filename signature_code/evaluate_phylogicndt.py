...
def score1C(sim, cluster_table):
    ...
    est_w = cluster_table['Cluster_Assignment'] / cluster_table['Cluster_Assignment'].sum()
...
if __name__ == '__main__':
    ...
    """
    folder_path = "20200210_simulations_clonesig_cn_cancer_type/type28-perc_diploid40-nb_clones6-nb_mut300"
    start_time = 100
    end_time = 234
    """
    ...
    loci_table = pd.read_csv('{}/phylogicndt/Test_Clust.mut_ccfs.txt'.format(folder_path), sep='\t')
    cluster_table = pd.read_csv('{}/phylogicndt/Test_Clust.cluster_ccfs.txt'.format(folder_path), sep='\t')
    cluster_table = pd.merge(cluster_table, loci_table.Cluster_Assignment.value_counts().to_frame(), left_on='Cluster_ID', right_index=True)
    ...
    id_list.append([cancer_type, perc_diploid, nb_clones, nb_mut,
                    ...
                    avg_major_cn, actual_perc_diploid, avg_tot_cn, 'PhylogicNDT', "all", np.nan])
    sml = list()
    sml.append(len(cluster_table))
    sml.append(np.nan)
    sml.append(np.nan)
    sml.append(score1B(sim_data_obj, cluster_table[cluster_table['Cluster_Assignment']>1]))
    sml.append(score1C(sim_data_obj, cluster_table))
    sml.append(score2A(sim_data_obj, loci_table.sort_values(by='Start_position')))
    auc, accuracy, sensitivity, specificity, precision = score2C(sim_data_obj, loci_table.sort_values(by='Start_position'), cluster_table)
    ...
    new_res_df = pd.concat([id_df, metrics_df], axis=1)
    old_res_df = pd.read_csv('{}/eval_phylogicndt.tsv'.format(folder_path), sep='\t')
    new_res_df.loc[new_res_df.method=='PhylogicNDT', 'runtime'] = old_res_df.runtime.mean()
    new_res_df.to_csv('{}/eval_phylogicndt.tsv'.format(folder_path), sep='\t',
                  index=False)
