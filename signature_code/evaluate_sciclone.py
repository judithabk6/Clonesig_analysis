...
loci_table = pd.read_csv('{}/sciclone/clusters1'.format(folder_path), sep='\t')
...
def score1C(sim, loci_table):
    ...
    est_w = loci_table[loci_table.cluster > 0].groupby('cluster')['tumor.vaf'].count()/len(loci_table)
    est_phi = loci_table[loci_table.cluster > 0].groupby('cluster')['tumor.vaf'].mean()/100
...
id_list.append([cancer_type, perc_diploid, nb_clones, nb_mut,
                ...
                avg_major_cn, actual_perc_diploid, avg_tot_cn, 'sciclone', "all", np.nan])
sml = list()
sml.append(loci_table[loci_table.cluster > 0].cluster.nunique())
sml.append(np.nan)
sml.append(np.nan)
...
res_df.to_csv('{}/eval_sciclone.tsv'.format(folder_path), sep='\t',
              index=False)
