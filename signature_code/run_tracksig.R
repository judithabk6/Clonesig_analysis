...
suffix = '_cancertype'
start_time <- Sys.time()
list[changepoints, mixtures] <- find_changepoints_pelt(vcf, subMU)
end_time <- Sys.time()
...
suffix = '_all'
start_time <- Sys.time()
list[changepoints, mixtures] <- find_changepoints_pelt(vcf, MU)
end_time <- Sys.time()
runtime = end_time - start_time
write.csv(mixtures, file=paste(folder_path, 'tracksig', paste("tracksig_mixtures", suffix, ".csv", sep=''), sep='/'))
n_col <- ifelse(length(changepoints) > 0, length(changepoints), 1)
...
suffix = '_prefit'
start_time <- Sys.time()
premixtures <- fit_mixture_of_multinomials_EM(rowSums(vcf), MU)
