...
cluster_table = pd.read_csv('{}/pyclone/tables/cluster.tsv'.format(folder_path), sep='\t')
loci_table = pd.read_csv('{}/pyclone/tables/loci.tsv'.format(folder_path), sep='\t')
...
def score2A(sim, loci_table):
    ...
    ordered_loci_table = pd.merge(true_df, loci_table, on='mutation_id')
...
id_list.append([cancer_type, perc_diploid, nb_clones, nb_mut,
                ...
                avg_major_cn, actual_perc_diploid, avg_tot_cn, 'pyclone', "all", np.nan])
sml = list()
sml.append(len(cluster_table))
sml.append(np.nan)
sml.append(np.nan)
sml.append(score1B(sim_data_obj, cluster_table[cluster_table['size']>1]))
...
res_df.to_csv('{}/eval_pyclone.tsv'.format(folder_path), sep='\t',
              index=False)
