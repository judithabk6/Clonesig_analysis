...
def score1C(sim, data_df):
    ...
    est_w = data_df.groupby('cluster_id').phi.count().values/len(data_df)
    est_phi = data_df.groupby('cluster_id').phi.mean().values
...
def score2C(sim, data_df):
    ...
    est_clonal_idx = data_df.groupby('cluster_id').phi.mean().idxmax()
...
if __name__ == '__main__':
    ...
    folder_path = "20200210_simulations_clonesig_cn_cancer_type/type0-perc_diploid40-nb_clones4-nb_mut5000"
    folder_path = "20200210_simulations_clonesig_cn_cancer_type/type3-perc_diploid40-nb_clones5-nb_mut1000"
    """
    ...
    for setting in ('all', 'cancertype', 'prefit'):
        ...
        for v in (auc, accuracy, sensitivity, specificity, precision):
            sml.append(v)
        mut_sig = np.moveaxis(
            np.repeat([mixture_file.values[:, 1:].T[data_df.bin-1]],
        ...
        data_df = data_df.assign(old_index=data_df.mutation_id.str.split('_').str[1].astype(int))
        sml.append(score_sig_1D(sim_data_obj, signature_mut[np.argsort(data_df.old_index)],  mu_mat[setting],
                                cancer_type, fitted_sigs))
        est_mut_profile = mixture_file.values[:, 1:].T[data_df.bin-1].dot(mu_mat[setting])
        (min_diff_distrib_mut, max_diff_distrib_mut, std_diff_distrib_mut,
            ...
            sim_data_obj, est_mut_profile[np.argsort(data_df.old_index), :].astype(float))
        for v in (min_diff_distrib_mut, max_diff_distrib_mut, std_diff_distrib_mut,
                  median_diff_distrib_mut, perc_dist_5, perc_dist_10):
            sml.append(v)
        fit_time = pd.read_csv('{}/tracksigfreq/tracksigfreq_runtime_{}.csv'.format(folder_path, setting),
    ...
    res_df.to_csv('{}/eval_tracksigfreq.tsv'.format(folder_path), sep='\t',
                  index=False)
